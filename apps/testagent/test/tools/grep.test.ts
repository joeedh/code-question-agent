...
describe("grepTool", () => {
  ...
  it("finds matches recursively by default", async () => {
    const result = await grepTool.run({ pattern: "findme" }, { workspaceDir });
  ...
  it("reports 'no matches' when nothing matches", async () => {
    const result = await grepTool.run({ pattern: "nope-not-here" }, { workspaceDir });
  ...
  it("restricts to the given path filter", async () => {
    const result = await grepTool.run({ pattern: "findme", path: "sub" }, { workspaceDir });
